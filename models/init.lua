...
function M.shareGradInput(model)
   ...
   -- Share gradInput for memory efficient backprop
     ...
     model:apply(function(m)
         ...
         if torch.isTensor(m.gradInput) and moduleType ~= 'nn.ConcatTable' and moduleType ~= 'nn.CMaxTable'then
      ...
      for i, m in ipairs(model:findModules('nn.ConcatTable')) do
         if m.__shareGradInputKey == 'basicconcat' then
		 if cache[i % 4] == nil then
		    cache[i % 4] = torch.CudaStorage(1)
		end
		 m.gradInput = torch.CudaTensor(cache[i % 4], 1, 0)
        end
      end

     for i, m in ipairs(model:findModules('nn.ConcatTable')) do
         
         if m.__shareGradInputKey == 'wayconcat' then
		 if cache['wayconcat'] == nil then
		    cache['wayconcat'] = torch.CudaStorage(1)
		end
		 m.gradInput = torch.CudaTensor(cache['wayconcat'], 1, 0)
        end
     end

     for i, m in ipairs(model:findModules('nn.CMaxTable')) do
         
         if m.__shareGradInputKey == 'maxtable' then
		 if cache['max1'] == nil and cache['max2'] == nil then
		    cache['max1'] = torch.CudaStorage(1)
	            cache['max2'] = torch.CudaStorage(1)
		end
		 m.gradInput[1] = torch.CudaTensor(cache['max1'], 1, 0)
	         m.gradInput[2] = torch.CudaTensor(cache['max2'], 1, 0)
        end
     end
